...
public class Layer {
    ...
    {
    	//All code was based off of the TA's lecture in lab
    	//This method is an implementation of the first equation of "The Unit Error(Delta) Value" slide from Dr. Noelle's lecture #25
    	
    	Vector error = targ.difference(act);//This is a Java translation of (Ti - yi), in other words, expected output - actual output
    ...
    {
    	//All code was based off of the TA's lecture in lab
    	//This method is an implementation of the second equation of "The Unit Error(Delta) Value" slide from Dr. Noelle's lecture #25
    	//This method is much like the computeOutputDelta() method, only differing in that the hidden layers do not have an expected value
    	...
    	for(Projection p : outputs)//Cycles through all of the output projections
    	{
    		...
    		//Since the equation on the lecture slide is a vector multiplied by a matrix, and the given product function is a multiplication of matrix by a vector
    		//The matrix must be transposed in order to maintain equality, as matrix multiplication is not commutative
    		error = error.sum(temp);//Undertakes the Sigma notation of summing all of the matrix multiplications, as specified by the equation on the lecture slide
    	}
